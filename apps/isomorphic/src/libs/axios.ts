...
export const apiClient: ApiClient = {
  ...
  getWithMeta: async <T = undefined, M = undefined>(
    ...
    const response = await baseApiClient.get<ApiResponseWithMeta<T, M>>(
      url,
      config
    );
  ...
  post: async <T = undefined>(
    ...
    const response = await baseApiClient.post<ApiResponse<T>>(
      url,
      data,
      config
    );
  ...
  patch: async <T = undefined>(
    ...
    const response = await baseApiClient.patch<ApiResponse<T>>(
      url,
      data,
      config
    );
